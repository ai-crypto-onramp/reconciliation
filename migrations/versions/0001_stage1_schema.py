...
# Enum-like value sets (kept in sync with reconciliation.db.models).
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "source",
            "external_event_id",
            name="uq_external_events_source_ext_id",
        ),
        sa.CheckConstraint(
            f"source IN {_txt_tuple(SOURCE_TYPES)}", name="ck_external_events_source"
        ),
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            f"source IN {_txt_tuple(SOURCE_TYPES)}", name="ck_recon_runs_source"
        ),
        sa.CheckConstraint(
            f"status IN {_txt_tuple(RUN_STATUSES)}", name="ck_recon_runs_status"
        ),
    )
    op.create_index(
        "ix_recon_runs_source_status", "recon_runs", ["source", "status"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "run_id",
            sa.BigInteger(),
            sa.ForeignKey("recon_runs.id", ondelete="SET NULL"),
            nullable=True,
        ),
        ...
        sa.Column("internal_amount", sa.Numeric(precision=28, scale=8), nullable=True),
        sa.Column("external_amount", sa.Numeric(precision=28, scale=8), nullable=True),
        ...
        sa.CheckConstraint(
            f"classification IN {_txt_tuple(BREAK_CLASSIFICATIONS)}",
            name="ck_breaks_classification",
        ),
        sa.CheckConstraint(
            f"status IN {_txt_tuple(BREAK_STATUSES)}", name="ck_breaks_status"
        ),
        sa.CheckConstraint(
            f"source IN {_txt_tuple(SOURCE_TYPES)}", name="ck_breaks_source"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "break_id",
            sa.BigInteger(),
            sa.ForeignKey("breaks.id", ondelete="CASCADE"),
            nullable=False,
        ),
        ...
        sa.CheckConstraint(
            f"type IN {_txt_tuple(RESOLUTION_TYPES)}", name="ck_break_resolutions_type"
        ),
    )
    op.create_index(
        "ix_break_resolutions_break_id", "break_resolutions", ["break_id"], unique=False
    )
    op.create_index(
        "ix_break_resolutions_created_at", "break_resolutions", ["created_at"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "match_strategy", sa.String(length=32), nullable=False, server_default="exact"
        ),
        sa.Column("tolerance_seconds", sa.Integer(), nullable=False, server_default="300"),
        sa.Column(
            "escalation_age_minutes", sa.Integer(), nullable=False, server_default="60"
        ),
        sa.Column(
            "auto_resolve_timing",
            sa.Boolean(),
        ...
        sa.CheckConstraint(
            f"match_strategy IN {_txt_tuple(MATCH_STRATEGIES)}",
            name="ck_recon_rules_match_strategy",
        ),
        sa.CheckConstraint(
            f"source IN {_txt_tuple(SOURCE_TYPES)}", name="ck_recon_rules_source"
        ),
        sa.CheckConstraint(
            "tolerance_seconds >= 0", name="ck_recon_rules_tolerance_nonneg"
        ),
        sa.CheckConstraint(
    ...
    op.create_index(
        "ix_recon_rules_source_asset", "recon_rules", ["source", "asset"], unique=False
    )
